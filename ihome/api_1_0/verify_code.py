...
def get_image_code(image_code_id):
    ...
    try:
        redis_store.setex('image_code_%s' % image_code_id,
                          constants.REDIS_IMAGE_EXPIRES, text)
...
@api.route("smscodes/<re('1[3|5|7|8]\\d{9}'):mobile>/")
def get_smscode(mobile):
    ...
    try:
        ...
        if send_flag:
            return jsonify(errno=RET.REQERR, errmsg='手机发送短信过于频繁,请60s后再试')
    ...
    send_sms(
        mobile,
        [sms_code, int(constants.REDIS_SMS_CODE_SEND_EXPIRES / 60)], 1)
    return jsonify(errno=RET.OK, errmsg='发送成功')
